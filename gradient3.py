...
for monte in range(montemonte):
    ...
    gsum_list = np.zeros((trajectory,step_size))
    g2sum_list = np.zeros((trajectory,step_size))
    ratio_list2 = np.zeros((trajectory,step_size))
    cumulative_raito_list2= np.zeros((trajectory,step_size))
    print theta
    ### IPW estimator
    for i in range(trajectory):
        x = 0.0 
        ...
        for t in range(step_size):
            x = x_list[i,t]
            a = a_list[i,t]
            ratio_list2[i,t] = norm.pdf(a,theta*x, sigma)/norm.pdf(theta*x,theta_b*x, sigma2)
            ###print ratio_list[i,t]
            cute = cute + np.log(norm.pdf(a,theta*x, sigma)/norm.pdf(theta*x,theta_b*x, sigma2))
            cumulative_raito_list2[i,t] = cute### + np.log(norm.pdf(a,theta*x,sigma)/norm.pdf(a,x,sigma))


    ### Marginal Ratio 
    marginal_raito_list2 = np.zeros((trajectory,step_size))
    for t in range(step_size):
        x1 = x_list[:,t] 
        x2 = x_list[:,t]*x_list[:,t]
        x3 = x_list[:,t]*x_list[:,t]*x_list[:,t]
        ...
        reg = linear_model.LinearRegression()
        reg.fit(x,y)
        marginal_raito_list2[:,t]= reg.predict(x)*ratio_list[:,t]

    ### Marginal Derivative 
  
    marginal_derivative_list2 = np.zeros((trajectory,step_size))
    for t in range(step_size):
        x1 = x_list[:,t] 
        x2 = x_list[:,t]*x_list[:,t]
        x3 = x_list[:,t]*x_list[:,t]*x_list[:,t]
        ...
        reg = linear_model.LinearRegression()
        reg.fit(x,y)
        marginal_derivative_list2[:,t]= reg.predict(x)


    ### Q Monte Carlo
    Q_estimator_list2 = np.zeros((trajectory,step_size))
    V_estimator_list2 = np.zeros((trajectory,step_size))
    reg_list = []
    for t_ in range(step_size):
        t = step_size-t_-1
        x1 = x_list[:,t]
        ...
        reg_list.append(np.append(reg.intercept_, reg.coef_))
        ####print r2_score(y,reg.predict(x))
        Q_estimator_list2[:,t] = reg.predict(x)
        x = np.transpose(np.array([x1,x2,x1,x2]))
        V_estimator_list2[:,t] = reg.predict(x)
        
    ### Q Monte Carlo Derivative 

    Q_estimator_derivative_list2 = np.zeros((trajectory,step_size))
    V_estimator_derivative_list2 = np.zeros((trajectory,step_size))
    for t_ in range(step_size):
        t = step_size-t_-1
        x1 = x_list[:,t]
        x2 = x_list[:,t]*x_list[:,t]
        ...
        reg.fit(x,y)
        ####print r2_score(y,reg.predict(x) )
        Q_estimator_derivative_list2[:,t] = reg.predict(x)

        mumu = theta*x_list[:,t]
        x = np.transpose(np.array([x1,x2,x1,x2]))
        V_estimator_derivative_list2[:,t] += reg.predict(x)

     ### REINFORCE  

    ipw_list = []
    for i in range(trajectory):
        ipw_list.append(np.sum(np.exp(cumulative_raito_list2[i,:])*y_list[i,:]*gsum_list[i,:]))
    ipw_list = np.array(ipw_list)
    gradient = np.mean(ipw_list)
    print(gradient)
    theta_mse[0,monte] =  gradient
    ...
    pg_list = []
    for i in range(trajectory):
        pg_list.append(np.sum(np.exp(cumulative_raito_list2[i,:])*g_list[i,:]*Q_estimator_list2[i,:]))
    pg_list = np.array(pg_list)
    gradient = np.mean(pg_list)
    print(gradient)
    #####theta_mse[0,monte] =  gradient
    ...
    for t in range(step_size):
        if t !=0:
            auxi[:,t] +=  np.exp(cumulative_raito_list2[:,t])*gsum_list[:,t]*(y_list[:,t]-Q_estimator_list2[:,t])
            auxi[:,t] +=  -np.exp(cumulative_raito_list2[:,t])*Q_estimator_derivative_list2[:,t]
            auxi[:,t] +=  np.exp(cumulative_raito_list2[:,t-1])*V_estimator_derivative_list2[:,t]
            auxi[:,t] +=  np.exp(cumulative_raito_list2[:,t-1])*gsum_list[:,t-1]*V_estimator_list2[:,t]
        else:
            auxi[:,t] +=  np.exp(cumulative_raito_list2[:,t])*gsum_list[:,t]*(y_list[:,t]-Q_estimator_list2[:,t])
            auxi[:,t] +=  -np.exp(cumulative_raito_list2[:,t])*Q_estimator_derivative_list2[:,t]                                                                                                                          
            auxi[:,t] +=  V_estimator_derivative_list2[:,t]
    db_estimator = np.sum(auxi,1)
    gradient = np.mean(db_estimator)
    print(gradient)
    theta_mse[2,monte] =  gradient
    ...
    for t in range(step_size):
        if t !=0:
            auxi[:,t] +=  marginal_derivative_list2[:,t]*(y_list[:,t]-Q_estimator_list2[:,t])
            auxi[:,t] +=  -marginal_raito_list2[:,t]*Q_estimator_derivative_list2[:,t]
            auxi[:,t] +=  marginal_raito_list2[:,t-1]*V_estimator_derivative_list2[:,t]
            auxi[:,t] +=  marginal_derivative_list2[:,t-1]*V_estimator_list2[:,t]
        else:
            auxi[:,t] +=  marginal_derivative_list2[:,t]*(y_list[:,t]-Q_estimator_list2[:,t])
            auxi[:,t] +=  -marginal_raito_list2[:,t]*Q_estimator_derivative_list2[:,t]                                                                                                                          
            auxi[:,t] +=  V_estimator_derivative_list2[:,t]
    ...
    np.savez("result/MSE_list3_1",x=theta_mse)
